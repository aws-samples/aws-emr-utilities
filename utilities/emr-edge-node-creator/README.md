...
Some users run edge node client in a docker container environment, such as in Amazon EKS or ECS. 
...
3. Create an EC2 instance to host one or multiple EMR Edge Node client containers (EC2 Console or CLI)
...
# create the edge node (replace variable placeholders)
...
```
# run one or multiple docker containers. the gz file in S3 should be pre-generated by step 1
...
5. test apps on a edge node container
```

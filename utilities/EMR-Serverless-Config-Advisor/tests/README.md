...
- `fixtures/b12/` — 12 production jobs (proven EMR Serverless configs,
  13.6% cheaper / 30.7% faster than the EC2 source)
- `fixtures/ec2_migration/` — 7 EC2-to-Serverless migration jobs, including
  shuffle-congestion-collapse cases that motivated the two-constraint sizing
  model (EC2 source, the proven-good run, the failed run, the cancelled run,
  and pipeline jobs that must NOT regress)

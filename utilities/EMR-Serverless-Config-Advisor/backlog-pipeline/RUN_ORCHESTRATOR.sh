#!/bin/bash
# ============================================================================
# Run EMR Serverless Orchestrator - Job Submission Script (TEST)
# ============================================================================
# Submits EMR Serverless jobs for unprocessed event logs
# TEST CONFIG: Processes up to 10 jobs (50 logs @ 5 logs per job)
...
# Format: serverless_advisor_orchestrator_pipeline_{ddmmyyHHMM}.log
LOG_TIMESTAMP=$(date '+%d%m%y%H%M')
LOG_FILE="serverless_advisor_orchestrator_pipeline_${LOG_TIMESTAMP}.log"
...
echo "=========================================================================="
echo "EMR Serverless Orchestrator - Job Submission (TEST)"
echo "=========================================================================="
...
# Job Submission Configuration (TEST: 10 jobs x 5 logs = 50 logs total)
export MAX_JOBS_PER_RUN="10"  # TEST LIMIT: Max 10 jobs per run (10 x 5 = 50 logs)
export MAX_CONCURRENT_JOBS="10"  # Max jobs to submit
export SUBMISSION_DELAY_SECONDS="20"  # Brief API rate-limit pause every 100 submissions
export LONG_SLEEP_AFTER_JOBS="250"  # After this many jobs, take a long sleep
...
echo ""
echo "TEST Batch Processing Mode:"
...
echo "  Max Jobs Per Run:     ${MAX_JOBS_PER_RUN} jobs (TEST LIMIT: ${MAX_JOBS_PER_RUN} x 5 = $((MAX_JOBS_PER_RUN * 5)) logs max)"
echo ""
...
echo "  Max Concurrent Jobs:  ${CAPACITY_MAX_CONCURRENT}"
...
if [ ! -f "$ORCHESTRATOR_SCRIPT" ]; then
    ...
    python3 <<'DOWNLOAD_SCRIPT' || { echo "Failed to download orchestrator from S3"; exit 1; }
import boto3
import os
import sys

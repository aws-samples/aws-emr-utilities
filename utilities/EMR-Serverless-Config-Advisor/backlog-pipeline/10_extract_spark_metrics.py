...
def extract_job_id_for_application(output_path, application_id):
    ...
    try:
        if output_path.startswith("s3://"):
            # S3 mode - check both spark_config_extract and task_stage_summary files
            import boto3
            parts = output_path.replace("s3://", "").split("/", 1)
            bucket = parts[0]
            prefix = parts[1].rstrip("/") + "/" if len(parts) > 1 else ""
            ...
            # Try spark_config_extract first (primary location)
            s3_key_config = f"{prefix}spark_config_extract/{application_id}.json"
            print(f"  DEBUG: Reading s3://{bucket}/{s3_key_config}")

            try:
                body = s3.get_object(Bucket=bucket, Key=s3_key_config)["Body"].read()
                data = json.loads(body)
                job_id = extract_job_id_from_json(data)
                if job_id:
                    print(f"  ✓ Extracted job_id: {job_id} from spark_config_extract/{application_id}.json")
                else:
                    print(f"  ⚠ No job_id in spark_config_extract/{application_id}.json")
            except s3.exceptions.NoSuchKey:
                print(f"  ⚠ File not found: s3://{bucket}/{s3_key_config}")
            except Exception as e:
                print(f"  ✗ Error reading {s3_key_config}: {e}")

            # If job_id not found, try task_stage_summary (fallback)
            if not job_id:
                s3_key_summary = f"{prefix}task_stage_summary/{application_id}.json"
                print(f"  DEBUG: Fallback - Reading s3://{bucket}/{s3_key_summary}")

                try:
                    body = s3.get_object(Bucket=bucket, Key=s3_key_summary)["Body"].read()
                    data = json.loads(body)
                    job_id = extract_job_id_from_json(data)
                    if job_id:
                        print(f"  ✓ Extracted job_id: {job_id} from task_stage_summary/{application_id}.json")
                    else:
                        print(f"  ⚠ No job_id in task_stage_summary/{application_id}.json")
                except s3.exceptions.NoSuchKey:
                    print(f"  ⚠ File not found: s3://{bucket}/{s3_key_summary}")
                except Exception as e:
                    print(f"  ✗ Error reading {s3_key_summary}: {e}")
        else:
            # Local mode - check both files
            ...
            # If job_id not found, try task_stage_summary (fallback)
            if not job_id:
                summary_file = f"{output_path}/task_stage_summary/{application_id}.json"
                print(f"  DEBUG: Fallback - Reading {summary_file}")
...
def extract_job_id_from_json(data):
    """
    Extract job_id from JSON data, checking multiple possible locations:
    1. Top level: data["job_id"]
    2. Inside application_info: data["application_info"]["job_id"]
    3. Inside spark_configuration: data["spark_configuration"]["spark.job_id"]
    """
    # Try top level first

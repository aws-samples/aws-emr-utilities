...
#   - Runs both jobs independently (job 2 runs even if job 1 fails)
#   - Captures all logs to S3
#   - Returns combined exit status
#
# Usage:
#   ./RUN_HOURLY_PIPELINE.sh
#
# Schedule with cron (every hour at minute 0):
...
# Master pipeline log
PIPELINE_LOG="${LOG_DIR}/hourly_pipeline_${RUN_TIMESTAMP}.log"

# S3 destination for logs
S3_LOG_BUCKET="${S3_BUCKET}"
S3_LOG_PREFIX="pipeline-files-v1/backlog-scale-dw/hourly_pipeline_logs"
S3_PIPELINE_LOG_DEST="s3://${S3_LOG_BUCKET}/${S3_LOG_PREFIX}/pipeline_${RUN_TIMESTAMP}.log"

# Start logging
exec > >(tee -a "${PIPELINE_LOG}") 2>&1

echo "=========================================================================="
echo "HOURLY PIPELINE EXECUTOR"
echo "=========================================================================="
...
# ============================================================================
# STEP 2: Run Orchestrator Production
# ============================================================================

echo ""
echo "=========================================================================="
echo "STEP 2: ORCHESTRATOR PRODUCTION"
CURRENT_STEP="STEP 2: EMR Serverless orchestrator"
echo "=========================================================================="
echo "Start Time: $(date '+%Y-%m-%d %H:%M:%S')"
echo ""
...
echo "Recent pipeline runs in S3:"
python3 -c "
import boto3
s3 = boto3.client('s3', region_name='${AWS_REGION}')
resp = s3.list_objects_v2(Bucket='${S3_LOG_BUCKET}', Prefix='${S3_LOG_PREFIX}/')
objs = [o for o in resp.get('Contents', []) if 'pipeline_' in o['Key'] and o['Key'].endswith('.log')]

...
S3_ENTRYPOINT_SCRIPT = f"s3://{S3_BUCKET}/{S3_SCRIPTS_PREFIX}/03_emr_serverless_single_job.py"
...
CAPACITY_MAX_CONCURRENT = int(os.getenv("CAPACITY_MAX_CONCURRENT", "500"))   # Max concurrent running jobs
CAPACITY_CHECK_INTERVAL = int(os.getenv("CAPACITY_CHECK_INTERVAL", "30"))    # Seconds to wait between rechecks when full
...
def submit_emr_serverless_job(emr_client, application_id, execution_role, job_name, log_batch):
    ...
    try:
        ...
        # Fixed resource allocation for all jobs (simplified configuration)
        # CRITICAL: Ensure minExecutors <= initialExecutors <= maxExecutors
        # Same configuration regardless of file size for consistency
        #
        # Executor Config: 2 vCPU + 8GB per executor (custom, not default)
        # Per job at startup: 1 driver (4 vCPU, 16GB) + 2 executors (4 vCPU, 16GB) = 8 vCPU, 32GB
        # Per job at max:     1 driver (4 vCPU, 16GB) + 3 executors (6 vCPU, 24GB) = 10 vCPU, 40GB
        # With 5 logs per job: 500 jobs to process 2500 logs
        # Concurrent capacity: 5000 vCPU / 8 vCPU = ~625 concurrent jobs ✅
        min_executors = 2
        initial_executors = 2
        max_executors = 3

        # Set profile for logging only
...
def submit_jobs_for_unprocessed_logs(unprocessed_logs, application_id, execution_role, max_concurrent):
    ...
    for idx, log_batch in enumerate(log_batches, 1):
        ...
        # Check if we've hit max concurrent limit (honours the MAX_CONCURRENT_JOBS cap)
        if len(submitted_jobs) >= max_concurrent and idx < len(log_batches):
            print(f"\n⚠ Reached MAX_CONCURRENT_JOBS limit ({max_concurrent})")
            print(f"  Submitted {len(submitted_jobs)} jobs so far")
            print(f"  Remaining batches: {len(log_batches) - idx}")
            print(f"  To submit more, increase MAX_CONCURRENT_JOBS or run again after jobs complete\n")
            break

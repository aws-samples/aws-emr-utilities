...
def load_json_files(path: str, limit: int = 500) -> List[Dict]:
    ...
    if is_s3_path(path):
        ...
        log.info("Loading from S3: s3://%s/%s (region: %s)", bucket, prefix, AWS_REGION)
...
def generate_dual_recommendations(input_path: str, limit: int = 500,
    ...
    log.info("Starting dual recommendations generation")
    ...
    try:
        ...
        log.error("Failed to load JSON files from %s: %s", input_path, e)
        raise RuntimeError(f"Failed to load metrics: {e}") from e

    if not all_data:
        log.warning("No data loaded from %s", input_path)
        return [], []
    ...
    if not flattened:
        log.warning("No valid data to process")
        return [], []

    try:
        df = pd.DataFrame(flattened)
    except Exception as e:
        log.error("Failed to create DataFrame: %s", e)
    ...
    for idx, row in df_with_data.iterrows():
        try:
            ...
            log.warning("Failed to extract basic metrics for row %s: %s", idx, e)
            continue
...
if __name__ == "__main__":
    ...
    parser.add_argument("--limit", type=int, default=500, help="Max applications")

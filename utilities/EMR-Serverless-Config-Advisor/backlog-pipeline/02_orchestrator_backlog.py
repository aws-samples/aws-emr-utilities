...
#
# Priority: Edit the variables, environment variables override if set
# Example: S3_BUCKET = "my-bucket" in script, export S3_BUCKET="other-bucket" wins
################################################################################
...
# This is where the orchestrator will download 03_spark_extractor.py, etc.
# Example: "pipeline-files/backlog-enabled"
S3_SCRIPTS_PREFIX = os.getenv("S3_SCRIPTS_PREFIX", "pipeline-files/backlog-enabled")
...
# Number of event logs to process per pipeline run
# Recommended: 500 for regular-sized logs, 5 for large logs (>1GB)
...
# Leave as default unless running multiple parallel instances
...
if S3_BUCKET == "YOUR_S3_BUCKET_HERE" or not S3_BUCKET:
    raise ValueError(
        ...
        "Please edit line 95 of this script and set your S3 bucket name.\n"
...
# Auto-derived settings (these use the configuration above)
...
def pull_batch_from_backlog(spark, backlog_table, batch_size, instance_id):
    ...
    try:
        ...
        eligible_count_df = spark.sql(f"""
            ...
            FROM {backlog_table}
            WHERE is_processed = 'N'
              AND (s3_file_size < 1073741824 OR s3_file_size IS NULL)
        """)
        eligible_count = eligible_count_df.collect()[0].cnt
        skipped_large = pending_count - eligible_count
        log(spark, f"Eligible logs (pending & < 1GB): {eligible_count:,}")
        if skipped_large > 0:
            log(spark, f"Skipped logs (>= 1GB): {skipped_large:,}")
...
def step_1_extract_metrics_from_backlog(spark, backlog_table, batch_size, instance_id):
    ...
    output_path = f"s3://{S3_BUCKET}/test-target-metrics/{TIMESTAMP}/"
